...
handler: StreamHandler = StreamHandler()  # pyright: ignore[reportMissingTypeArgument]
...
def tostr(s: bytes | str | object) -> str:
...
def parse_smtp_content(content: str) -> tuple[str, str | None]:
    ...
    Returns:
        tuple[str, str | None]: Tuple of message body and subject
    """
    ...
    subject: str | None = email_message.get('Subject')
...
class CustomHandler:
    ...
    async def handle_DATA(self, server: SMTP, session: Session, envelope: Envelope) -> str:
        ...
        tokens: dict[str, DiscordToken] = {
            k: DiscordToken(**v)
            for k, v in json.loads(Path('discord.json').read_text()).items()  # type: ignore
        }  # type: ignore
